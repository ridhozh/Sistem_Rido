            ...
            <div class="overflow-y-auto pr-2 custom-scrollbar" style="max-height: 700px;">
                ...
                <div class="category-section mb-12">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                        ...
                        <div class="group bg-white border border-slate-100 rounded-[2rem] p-3 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300">

                            <div class="relative w-full aspect-square rounded-[1.5rem] mb-4 overflow-hidden bg-slate-50">
                            ...
                            <div class="px-1 text-center sm:text-left">
                                <h3 class="text-[11px] font-bold text-slate-700 truncate mb-1">{{ $item->nama_produk }}</h3>
                                <p class="text-sm font-black text-blue-600">
                                    Rp {{ number_format($item->harga, 0, ',', '.') }}
                                </p>
                                <p class="text-[9px] text-slate-400 mt-1 italic">Stok: {{ $item->stok_awal }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <h2 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-3">
                    <div class="p-2 bg-blue-50 rounded-lg">
                        <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-width="2.5" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                        </svg>
                    </div>
                    Ringkasan
                </h2>
            ...
            <div class="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm">
                <div class="space-y-4">
                    <div class="grid grid-cols-1 gap-4">
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Metode Bayar</label>
                            <select class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none">
                                <option>Tunai</option>
                                <option>QRIS</option>
                            </select>
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Uang Bayar</label>
                            <input type="number" id="input-bayar" value="0" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black text-blue-600 outline-none">
                        </div>
                    </div>
                    ...
                    <button onclick="prosesTransaksi()" class="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-sm transition-all shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2">
                        PROSES TRANSAKSI
                    </button>
                </div>
            </div>
    ...
    <div class="lg:col-span-12 mt-4">
        <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
            <div class="flex items-center justify-between mb-8">
                ...
                <span id="jenis-produk-count" class="px-4 py-1.5 bg-slate-100 text-slate-600 rounded-full text-[10px] font-bold uppercase">0 Jenis Produk</span>
            </div>
                ...
                <table class="min-w-full divide-y divide-slate-100">
                    <thead>
                        <tr class="text-slate-400">
                            <th class="px-6 py-4 text-left text-[10px] font-bold uppercase tracking-widest">Nama Produk</th>
                            ...
                            <th class="px-6 py-4 text-left text-[10px] font-bold uppercase tracking-widest">Subtotal</th>
...
<script>
    ...
    function renderEverything() {
        ...
        if (cart.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-12 text-center text-slate-400 italic">Belum ada barang dipilih.</td></tr>';
    ...
    function prosesTransaksi() {
        ...
        // Here you would normally send the 'cart' array to Laravel via AJAX
    }
</script>
@endpush