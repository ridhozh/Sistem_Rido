...
<div class="fixed top-10 inset-x-0 z-[100] flex flex-col items-center pointer-events-none space-y-4">
    ...
    @if ($errors->any())
    <div id="toast-error" class="pointer-events-auto min-w-[320px] max-w-md bg-white/80 backdrop-blur-lg border border-red-200 shadow-2xl rounded-2xl overflow-hidden transform transition-all duration-500">
    ...
    @if (session('success'))
    <div id="toast-success" class="pointer-events-auto min-w-[320px] max-w-md bg-white/80 backdrop-blur-lg border border-emerald-200 shadow-2xl rounded-2xl overflow-hidden transform transition-all duration-500">
        ...
        <div class="h-1 bg-emerald-100 w-full">
            <div id="success-progress" class="h-1 bg-emerald-500 transition-all linear duration-[5000ms] w-full"></div>
        </div>
    </div>
    @endif
</div>
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        ...
                        <form action="{{ route('admin.produk.destroy', $produk->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Hapus produk ini?')">
                                Hapus
                            </button>
                        </form>
                    </td>
...
<div id="modal-produk" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg">
        ...
        <!-- Form di dalam Modal -->
        <form id="form-produk" action="{{ route('admin.produk.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div id="file-info" class="mt-2 text-xs text-gray-500 hidden italic">
                        <span class="text-blue-600 font-semibold">Catatan:</span> Sudah ada gambar tersimpan. Biarkan kosong jika tidak ingin mengubahnya.
                        <div id="preview-container" class="mt-1">
                            <img id="old-photo-preview" src="" class="h-10 w-10 object-cover rounded border shadow-sm hidden">
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const bukaModal = (mode = 'tambah', data = null) => {
            ...
            if (mode === 'edit' && data) {
                ...
                if (document.getElementById('produk-id')) document.getElementById('produk-id').value = data.id;
                if (document.getElementById('nama_produk')) document.getElementById('nama_produk').value = data.nama;
                if (document.getElementById('kategori')) document.getElementById('kategori').value = data.kategori_id;
                if (document.getElementById('harga')) document.getElementById('harga').value = data.harga;
                if (document.getElementById('stok_awal')) document.getElementById('stok_awal').value = data.stok;
